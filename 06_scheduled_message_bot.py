...
SCHEDULED_MESSAGES = [
    {'text': 'First message', 'post_at': int(
        (datetime.now() + timedelta(seconds=20)).timestamp()), 'channel': 'C038LBB65H8'},
    {'text': 'Second Message!', 'post_at': int(
        (datetime.now() + timedelta(seconds=30)).timestamp()), 'channel': 'C038LBB65H8'}
]
...
if __name__ == "__main__":
    ...
    ids = list_scheduled_messages('C038LBB65H8')
    delete_scheduled_messages(ids, 'C038LBB65H8')
